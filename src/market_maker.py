...
class Asset:
    def __init__(self, initial_dividend: float, ro: float, alpha: float, bank_return: float, supply: int):
    ...
    def update_dividend(self) -> float:
        delta = self._ro * self._prev_dividend_delta + self._alpha * np.random.normal(loc=0, scale=1)
...
class Auction:
    ...
    def add_demand(self, amount: int):
        self._demand += amount
